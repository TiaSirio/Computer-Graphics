...
struct Model {
	...
	void createTextModel(std::string file);
	void createIndexBuffer();
	void createVertexBuffer();

	void init(BaseProject *bp, std::string file, bool text);
	void cleanup();
};
...
struct Pipeline {
	...
	VkPipeline graphicsPipeline;
  	...
  	void init(BaseProject *bp, const std::string& VertShader, const std::string& FragShader,
  			  std::vector<DescriptorSetLayout *> D, bool transp);
...
protected:
    ...
    void initVulkan() {
		...
		createColorResources();			//Multisampling
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
		...
		populateCommandBuffer(commandBuffer, imageIndex);
	...
	virtual void localPipelineCleanup() = 0;
	...
	virtual void localPipelineInit() = 0;
...
void Model::createTextModel(std::string file) {
	...
	//not sure
	vertices.resize(4 * vertices.size() * totLen);
		...
		for (int i = 0; i < Txt.usedLines; i++) {
			for (int j = 0; j < strlen(Txt.l[i]); j++) {
				...
				if ((c >= 0) && (c <= maxChar)) {
					...
					vertex[vb + 0] = (float)(tpx + d.xoffset) * PtoTsx + PtoTdx;
					vertex[vb + 1] = (float)(tpy + d.yoffset) * PtoTsy + PtoTdy;
					vertex[vb + 2] = 0.0;
					vertex[vb + 3] = (float)d.x / texW;
					vertex[vb + 4] = (float)d.y / texH;

					vertex[vb + 5] = (float)(tpx + d.xoffset + d.width) * PtoTsx + PtoTdx;
					vertex[vb + 6] = (float)(tpy + d.yoffset) * PtoTsy + PtoTdy;
					vertex[vb + 7] = 0.0;
					vertex[vb + 8] = (float)(d.x + d.width) / texW;
					vertex[vb + 9] = (float)d.y / texH;

					vertex[vb + 10] = (float)(tpx + d.xoffset) * PtoTsx + PtoTdx;
					vertex[vb + 11] = (float)(tpy + d.yoffset + d.height) * PtoTsy + PtoTdy;
					vertex[vb + 12] = 0.0;
					vertex[vb + 13] = (float)d.x / texW;
					vertex[vb + 14] = (float)(d.y + d.height) / texH;

					vertex[vb + 15] = (float)(tpx + d.xoffset + d.width) * PtoTsx + PtoTdx;
					vertex[vb + 16] = (float)(tpy + d.yoffset + d.height) * PtoTsy + PtoTdy;
					vertex[vb + 17] = 0.0;
					vertex[vb + 18] = (float)(d.x + d.width) / texW;
					vertex[vb + 19] = (float)(d.y + d.height) / texH;
...
void Model::init(BaseProject *bp, std::string file, bool text) {
	BP = bp;
	if (!text) {
		loadModel(file);
	}
	else {
		//createTextModel(file);
	}
	createVertexBuffer();
	createIndexBuffer();
}
...
void Texture::createTextureImage(std::string file) {
	...
	BP->createImage(texWidth, texHeight, mipLevels, VK_SAMPLE_COUNT_1_BIT,
...
void Pipeline::init(BaseProject *bp, const std::string& VertShader, const std::string& FragShader,
	...
	multisampling.sampleShadingEnable = VK_TRUE;					//improve the image quality
	multisampling.minSampleShading = .2f;							//min fraction for sample shading; closer to one is smoother
	//multisampling.rasterizationSamples = VK_SAMPLE_COUNT_1_BIT;
	multisampling.rasterizationSamples = BP->msaaSamples;			//Multisampling
